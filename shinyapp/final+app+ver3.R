...
setwd("~/Documents/GitHub/Spr2017-proj2-proj2_grp8/data")
...
server=shinyServer(function(input, output){
    ...
    map <- reactive({
        if (input$asp == 1){
            d <- read.csv("All-in/Population.csv")
        ...
        if(input$asp == 2 ){
            crime <- read.csv(paste("crime/",input$city,".csv",sep = ""))
            crime <- na.omit(crime)
        ...
        if(input$asp == 3){
            lib <- read.csv(paste("City Raw/",input$city, "/Library.csv", sep = ""))
            lib=
                lib%>%
                filter(lib$ZIP>0)%>%
        ...
        if (input$asp == 4){
            res <- read.csv(paste("City Raw/",input$city, "/Restaurant.csv", sep = ""))
            res=
                res%>%
                filter(res$ZIP>0)%>%
                mutate(region=as.character(ZIP))
        ...
        if (input$asp == 5){
            res <- read.csv(paste("City Raw/",input$city, "/Park.csv", sep = ""))
            res=
                res%>%
                filter(res$ZIP>0)%>%
                mutate(region=as.character(ZIP))
        ...
        if (input$asp == 6){
            res <- read.csv(paste("City Raw/",input$city, "/Health.csv", sep = ""))
            res=
                res%>%
                filter(res$ZIP>0)%>%
                mutate(region=as.character(ZIP))
    ...
    leafletmap <- reactive({
       ...
       if(!is.na(trans_ziplat(zip=zip_code()))){
        ...
        return(ny_map)
          ...
          addPolygons(options = pathOptions(),opacity=0.8,weight=1,color="#ffffb2",highlightOptions = highlightOptions(color="#006d2c",opacity = 0.5, weight = 2, fillOpacity = 0.5,
                                                                                                                       bringToFront = TRUE, sendToBack = TRUE),label=c_label) %>%
          addPolygons(data = highlight_3, options = pathOptions(),opacity=0.5,weight=1,color="#006d2c")%>%
          addPolygons(data = highlight_2, options = pathOptions(),opacity=0.5,weight=1,color="#253494")%>%
          addPolygons(data = highlight_1, options = pathOptions(),opacity=0.5,weight=1,color="#cb181d")
          return(ny_map)
        }
    })
    streetmap<-reactive({leaflet()%>%
      addTiles() %>%
      setView(lat=40.79850,lng=-73.96811,zoom=17)
    })
    output$leafmap <- renderLeaflet({
        leafletmap()
    })
    output$streetmap<-renderLeaflet({leaflet()%>%
        addTiles() %>%
        setView(lat=40.79850,lng=-73.96811,zoom=17)
    })
    ...
    observeEvent(input$map_click,{
      ...
      address <- revgeocode(c(area_click$lng,area_click$lat))
